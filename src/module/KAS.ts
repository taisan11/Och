...
function formatUnixTime(unixTime: number): string {
  ...
  // 時分秒を取得
  const hms = date.toLocaleTimeString('ja-JP', {
    hour: '2-digit',
    minute: '2-digit',
    second: '2-digit',
  })
...
function MES(input: string | null): string {
  ...
  // HTML特殊文字を変換
  const htmlEscaped = input.replace(
    /[&<>"']/g,
    match =>
      ({
        '&': '&amp;',
  ...
  const numLinkConverted = newlineEscaped.replace(
    /&gt;&gt;(\d+)/g,
    '<a href="#$1">&gt;&gt;$1</a>',
  )
...
async function NES(
  input: string,
  mail: string,
): Promise<{ name: string; mail: string }> {
  if (!input) {
