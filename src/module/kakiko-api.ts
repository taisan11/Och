 ...
 */
export async function kakikoAPI(
  {
    ThTitle,
    name,
    mail,
  ...
  c: Context,
  mode: 'newth' | 'kakiko',
): Promise<{ sc: boolean; ThID: string }> {
  if (mode === 'newth') {
    ...
    // 文字数制限など
    if (name.length > 30) {
      return { sc: false, ThID: `error0` }
    }
    if (!MESSAGE || MESSAGE.length > 300) {
      return { sc: false, ThID: 'error1' }
    }
    if (mail.length > 70) {
      return { sc: false, ThID: 'error2' }
    }
    if (!BBSKEY) {
      return { sc: false, ThID: 'error3' }
    }
    if (!ThTitle) {
      return { sc: false, ThID: 'error5' }
    }
    const ID = await id(IP, BBSKEY)
    // 加工
    const KASS = await KAS(MESSAGE, name, mail, Number(UnixTime))
    const a = await exic(1, {
      name: KASS.name,
      mail: KASS.mail,
      message: KASS.mes,
    })
    // 保存
    await NewThread(BBSKEY, {
      name: a.data.name || KASS.name,
      mail: a.data.mail || KASS.mail,
      message: a.data.message || KASS.mes,
      date: KASS.time + ' ID:' + ID,
      title: ThTitle,
      id: UnixTime,
    })
    // 返す
  ...
  if (mode === 'kakiko') {
    ...
    if (!ThID) {
      return { sc: false, ThID: 'error4' }
    }
    const ID = await id(IP, BBSKEY)
    // 変換
    const KASS = await KAS(MESSAGE, name, mail, Number(UnixTime))
    const a = await exic(2, {
      name: KASS.name,
      mail: KASS.mail,
      message: KASS.mes,
    })
    // 保存
    await postThread(BBSKEY, {
      name: a.data.name || KASS.name,
      mail: a.data.mail || KASS.mail,
      message: a.data.message || KASS.mes,
      date: KASS.time + ' ID:' + ID,
