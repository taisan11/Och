 ...
 */
export async function kakiko(c: Context, mode: 'newth' | 'kakiko'): Promise<{ sc: boolean, redirect: string }> {
    ...
    if (mode === 'newth') {
        ...
        // 文字数制限など
        if (Name.length > 30) { return { 'sc': false, 'redirect': `/error?e=0` } }
        if (!MESSAGE || MESSAGE.length > 300) { return { 'sc': false, 'redirect': `/error?e=1` } }
        if (mail.length > 70) { return { 'sc': false, 'redirect': `/error?e=2` } }
        if (!BBSKEY) { return { 'sc': false, 'redirect': `/error?e=3` } }
        if (!ThTi) { return { 'sc': false, 'redirect': `/error?e=5` } }
        // 加工
        ...
        // 返す
        return { 'sc': true, 'redirect': `/${BBSKEY}/${UnixTime}` };
    }
    if (mode === 'kakiko') {
        // 内容物の取得
        const Name = String(body.name);//名前
        ...
        if (mail.length > 70) { return {'sc':false,'redirect':`/error?e=2`} }
        if (!BBSKEY) { return {'sc':false,'redirect':`/error?e=3`} }
        if (!THID) { return {'sc':false,'redirect':`/error?e=4`} }
        ...
        return {'sc':true,'redirect':`/${BBSKEY}/${THID}`};
