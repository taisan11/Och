...
app.get(`/:BBSKEY`, async c => {
  ...
  return c.render(
    <>
      <h1>READ.CGI</h1>
      {Object.entries(SUBJECTJSON.data).map(
        ([unixtime, [threadName, responseCount]]) => {
          const date = new Date(Number.parseInt(unixtime) * 1000)
          const formattedDate = `${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`
          return (
            <>
              <a
                href={`./${BBSKEY}/${unixtime}`}
              >{`${threadName}-${responseCount}-${formattedDate}`}</a>
              <br />
            </>
          )
        },
...
app.post(`/:BBSKEY/:THID`, async c => {
  ...
  const kextuka = await kakikoAPI(
    { ThID, name, mail, MESSAGE, BBSKEY },
    c,
    'kakiko',
  )
...
app.post(`/:BBSKEY`, async c => {
  ...
  const BBSKEY = c.req.param('BBSKEY') //BBSKEY
  const kextuka = await kakikoAPI(
    { ThTitle, name, mail, MESSAGE, BBSKEY },
    c,
    'newth',
  )
  return c.redirect(kextuka.ThID)
