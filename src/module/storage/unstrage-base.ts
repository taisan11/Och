...
import { datpaser, subjectpaser } from '../pase'
import type {
  NewThreadParams,
  PostThreadParams,
  getSubjectReturn,
...
export async function addSubject_file(
  BBSKEY: string,
  postnum: number,
  title: string,
  id: string,
...
export async function getSubject_file(
  BBSKEY: string,
): Promise<getSubjectReturn> {
  const storage = createStorage(drives)
  const SUBTXT = await storage.getItem(`${BBSKEY}/SUBJECT.TXT`)
...
export async function NewThread_file(
  BBSKEY: string,
  { name, mail, message, date, title, id }: NewThreadParams,
): Promise<postReturn> {
  const storage = createStorage(drives)
  await storage.setItem(
    `${BBSKEY}/dat/${id}.dat`,
    `${name}<>${mail}<>${date}<>${message}<>${title}`,
  )
  await addSubject_file(BBSKEY, 1, title, id)
  return { sc: true, redirect: `${BBSKEY}/${id}` }
}

export async function postThread_file(
  BBSKEY: string,
  { name, mail, message, date, id }: PostThreadParams,
): Promise<postReturn> {
  const storage = createStorage(drives)
...
export async function getThread_file(
  BBSKEY: string,
  id: string,
): Promise<getThreadReturn> {
  const storage = createStorage(drives)
...
export async function getdat_file(
  BBSKEY: string,
  idextension: string,
): Promise<string> {
  const storage = createStorage(drives)
