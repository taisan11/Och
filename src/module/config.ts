...
import { type RuntimeName, runtimeInfo } from 'std-env'
import type { Driver } from 'unstorage'
import configa from '../../data/system.config'
export function config(): Config {
  configa.preference.site.use =
    runtimeInfo?.name || 'other' || configa.preference.site.use
  if (configa.preference.limit) {
    configa.preference.limit.MaxSubject =
      configa.preference.limit.MaxSubject || 20
  }
